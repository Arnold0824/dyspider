...
class kanxiPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        try:
            with self.connection.cursor() as cursor:
                # Create a new record
                cursor.execute("select count(*) as r from `core_film` where film_name=%s",str(item['title']))
                ...
                if rets[0]['r']==0:
                    ...
                    cursor.execute("update `core_film` set download_link=%s where `film_name`=%s",(str(item['downloadlink']),str(item['title'])))
                    self.connection.commit()
                # connection is not autocommit by default. So you must commit to save
                # your changes.
                self.connection.commit()
